...
const login = async (req, res) => {
  try {
    ...
    if (!admin) {
      // Always run bcrypt to prevent timing attacks that reveal whether email exists
    ...
    if (admin.lockUntil && admin.lockUntil > Date.now()) {
      return res.status(429).json(formatErrorResponse('Account is temporarily locked. Try again later.', 429));
    }
...
const resetPassword = async (req, res) => {
  try {
    ...
    if (!token) {
      return res.status(400).json(formatErrorResponse('Reset token is required'));
    }
