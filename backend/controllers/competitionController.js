...
const isCompetitionDebugEnabled =
  process.env.NODE_ENV === 'development' && process.env.DEBUG_COMPETITION === 'true';
const debugLog = (...args) => {
  if (isCompetitionDebugEnabled) {
    console.log(...args);
  }
};
...
const calculateRanks = (results) => {
  ...
  debugLog('Ranks calculated:', sortedResults.map(r => ({ 
    name: r.name, 
  ...
  if (resultsWithoutRanks.length > 0) {
    console.error(`calculateRanks: ${resultsWithoutRanks.length} result(s) missing ranks`);
  }
...
const getCompetitionStatus = async (req, res) => {
  try {
    ...
    const effectiveEntryFee = !isPaymentEnabled() && process.env.NODE_ENV !== 'production'
      ? 0
    ...
    // debugLog('getCompetitionStatus: Returning data:', responseData);
    // debugLog('getCompetitionStatus: resultsPublished value:', competition.resultsPublished);
...
const registerForCompetition = async (req, res) => {
  try {
    ...
    if (effectiveEntryFee > 0) {
      ...
      if (parsedAmount !== effectiveEntryFee) {
        return res.status(StatusCodes.BAD_REQUEST).json({
          success: false,
          message: `Payment amount must be ₹${effectiveEntryFee}`
        });
      }
      ...
      try {
        ...
        if (payment.status !== 'captured' || payment.amount !== Math.round(effectiveEntryFee * 100)) {
          return res.status(StatusCodes.BAD_REQUEST).json({
            success: false,
    ...
    const registrationData = {
      ...
      paymentStatus: effectiveEntryFee > 0 ? 'completed' : 'free'
    };
    ...
    res.status(StatusCodes.CREATED).json({
      success: true,
      message: effectiveEntryFee > 0 ? 'Registration successful' : 'Free registration successful',
      data: {
        secretId: registration.secretId,
        name: registration.name,
        mobile: registration.mobile,
        isFree: effectiveEntryFee === 0
      }
    });
  } catch (error) {
...
const submitCompetitionResult = async (req, res) => {
  try {
    debugLog('Competition result submission request:', req.body);
    ...
    const sanitizedSecretId = String(secretId).trim().replace(/[^\w-]/g, '');
    const sanitizedName = String(name).trim().slice(0, 100);

    if (!sanitizedSecretId || sanitizedSecretId.length > 64) {
      return res.status(StatusCodes.BAD_REQUEST).json({
        success: false,
    ...
    if (typeof netSpeed !== 'number' || netSpeed < 0) {
      debugLog('Validation failed: invalid netSpeed:', netSpeed);
      return res.status(StatusCodes.BAD_REQUEST).json({
        success: false,
    ...
    if (typeof accuracy !== 'number' || accuracy < 0 || accuracy > 100) {
      debugLog('Validation failed: invalid accuracy:', accuracy);
      return res.status(StatusCodes.BAD_REQUEST).json({
        success: false,
    ...
    // Enforce minimum test duration to prevent empty result submissions (30 seconds)
    if (timeTaken < 30) {
      return res.status(StatusCodes.BAD_REQUEST).json({
        success: false,
        message: 'Test must run for at least 30 seconds'
      });
    }
    ...
    // Find registration using sanitized secretId
    const registration = await CompetitionRegistration.findOne({
    ...
    // Check if name matches (use sanitized name)
    if (registration.name.toLowerCase() !== sanitizedName.toLowerCase()) {
      return res.status(StatusCodes.BAD_REQUEST).json({
        success: false,
        message: 'Name does not match with registration'
      });
    ...
    // Use a MongoDB session/transaction so both saves are atomic
    ...
    const userResult = competition.results.find(r => r.secretId === sanitizedSecretId);
...
const updateCompetitionSettings = async (req, res) => {
  try {
    debugLog('updateCompetitionSettings: Request received');
    debugLog('updateCompetitionSettings: Request body:', req.body);
    debugLog('updateCompetitionSettings: Request headers:', req.headers);
    ...
    debugLog('updateCompetitionSettings: Found competition:', {
      _id: competition?._id,
    ...
    if (!competition) {
      ...
      debugLog('updateCompetitionSettings: Updating existing competition');
      if (isRegistrationActive !== undefined) {
        debugLog('Updating isRegistrationActive from', competition.isRegistrationActive, 'to', isRegistrationActive);
        competition.isRegistrationActive = isRegistrationActive;
      }
      if (isCompetitionActive !== undefined) {
        debugLog('Updating isCompetitionActive from', competition.isCompetitionActive, 'to', isCompetitionActive);
      ...
      if (resultsPublished !== undefined) {
        debugLog('Updating resultsPublished from', competition.resultsPublished, 'to', resultsPublished);
    ...
    debugLog('updateCompetitionSettings: Competition saved successfully');
    debugLog('updateCompetitionSettings: Final resultsPublished value:', competition.resultsPublished);
...
const deleteAllRegistrations = async (req, res) => {
  try {
    ...
    debugLog('Deleting all registrations and results from competition:', competition._id);
    debugLog('Deleted registrations count:', result.deletedCount);
    debugLog('Results array length before deletion:', competition.results.length);
    debugLog('Competition object before save:', {
      _id: competition._id,
    ...
    debugLog('Registrations and results deleted successfully. New results array length:', competition.results.length);
    debugLog('Competition object after save:', {
      _id: competition._id,
      resultsLength: competition.results.length,
      totalRegistrations: competition.totalRegistrations,
      totalParticipants: competition.totalParticipants,
    ...
    debugLog('Verification after deletion (deleteAllRegistrations):', {
...
const deleteAllResults = async (req, res) => {
  try {
    ...
    debugLog('Deleting all results from competition:', competition._id);
    debugLog('Results array length before deletion:', competition.results.length);
    debugLog('Competition object before save:', {
      _id: competition._id,
      resultsLength: competition.results.length,
    ...
    debugLog('Results deleted successfully. New results array length:', competition.results.length);
    debugLog('Competition object after save:', {
      _id: competition._id,
      resultsLength: competition.results.length,
      totalParticipants: competition.totalParticipants,
    ...
    // Verify deletion was successful
    const verification = await Competition.findById(competition._id);
    debugLog('Verification after deletion (deleteAllResults):', {
      _id: verification._id,
      resultsLength: verification.results.length,
      totalParticipants: verification.totalParticipants,
      resultsPublished: verification.resultsPublished
...
const unpublishResults = async (req, res) => {
  try {
    debugLog('unpublishResults: Request received');
    ...
    debugLog('unpublishResults: Found competition:', {
      _id: competition._id,
      resultsPublished: competition.resultsPublished
    });
    ...
    debugLog('unpublishResults: Results unpublished successfully');
