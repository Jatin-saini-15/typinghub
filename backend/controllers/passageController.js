...
const isPassageDebugEnabled =
  process.env.NODE_ENV === 'development' && process.env.DEBUG_PASSAGES === 'true';
const debugLog = (...args) => {
  if (isPassageDebugEnabled) {
    console.log(...args);
  }
};
...
const getAllPassages = async (req, res) => {
  try {
    ...
    debugLog('Fetched all passages:', passages.length);
...
const getPassageById = async (req, res) => {
  try {
    ...
    if (!passage) {
      debugLog(`Passage not found: ${id}`);
      return res.status(404).json({ message: 'Passage not found' });
    }
    debugLog('Fetched passage by ID:', passage._id);
...
const createPassage = async (req, res) => {
  debugLog('Create passage called', { title: req.body.title, by: req.user ? req.user.email : (req.admin ? req.admin.email : 'unknown') });
  try {
    ...
    if (!titleValidation.isValid) {
      debugLog('Create passage failed:', titleValidation.message);
    ...
    if (!contentValidation.isValid) {
      debugLog('Create passage failed:', contentValidation.message);
    ...
    if (existingPassage) {
      debugLog('Create passage failed: Duplicate title');
    ...
    await passage.save();
    debugLog('Passage created:', passage._id);
...
const updatePassage = async (req, res) => {
  try {
    ...
    const titleValidation = validateTitle(title);
    if (!titleValidation.isValid) {
      debugLog('Update passage failed:', titleValidation.message);
      return res.status(400).json({ message: titleValidation.message });
    }
    ...
    const contentValidation = validateContent(content);
    if (!contentValidation.isValid) {
      debugLog('Update passage failed:', contentValidation.message);
      return res.status(400).json({ message: contentValidation.message });
    }
    ...
    const passage = await Passage.findById(id);
    if (!passage) {
      debugLog(`Passage not found for update: ${id}`);
      return res.status(404).json({ message: 'Passage not found' });
    }
    ...
    if (existingPassage) {
      debugLog('Update passage failed: Duplicate title');
      return res.status(409).json({ message: 'A passage with this title already exists' });
    }
    ...
    debugLog('Passage updated:', passage._id);
    res.status(200).json(passage);
  } catch (error) {
...
const deletePassage = async (req, res) => {
  try {
    ...
    const passage = await Passage.findById(id);
    if (!passage) {
      debugLog(`Passage not found for deletion: ${id}`);
      return res.status(404).json({ message: 'Passage not found' });
    }

    await passage.deleteOne();
    debugLog('Passage deleted:', id);
...
const getPassagesByTestType = async (req, res) => {
  try {
    ...
    if (!testType || typeof testType !== 'string') {
      debugLog('Get passages by test type failed: Valid testType required');
    ...
    debugLog(`Fetched passages for test type ${sanitizedTestType}:`, passages.length);
    res.status(200).json(passages);
  } catch (error) {
...
const assignPassage = async (req, res) => {
  try {
    ...
    if (!passageId || typeof passageId !== 'string') {
      debugLog('Assign passage failed: Valid passageId required');
      return res.status(400).json({ message: 'Valid passage ID is required' });
    }

    if (!categories || !Array.isArray(categories) || categories.length === 0) {
      debugLog('Assign passage failed: Valid categories array required');
    ...
    for (const category of sanitizedCategories) {
      ...
      debugLog('Assign passage failed: Invalid category');
    ...
    if (!passage) {
      debugLog(`Passage not found for assignment: ${passageId}`);
      return res.status(404).json({ message: 'Passage not found' });
    }
    ...
    if (newCategories.length > 0) {
      ...
      await passage.save();
      debugLog(`Passage ${passageId} assigned to categories: ${newCategories.join(', ')}`);
    } else {
      debugLog(`Passage ${passageId} already assigned to all requested categories`);
    }
...
const unassignPassage = async (req, res) => {
  try {
    ...
    if (!passageId || typeof passageId !== 'string') {
      debugLog('Unassign passage failed: Valid passageId required');
      return res.status(400).json({ message: 'Valid passage ID is required' });
    }

    if (!categories || !Array.isArray(categories) || categories.length === 0) {
      debugLog('Unassign passage failed: Valid categories array required');
      return res.status(400).json({ message: 'Valid categories array is required' });
    }
    ...
    for (const category of sanitizedCategories) {
      if (!validCategories.includes(category)) {
        debugLog('Unassign passage failed: Invalid category');
        return res.status(400).json({ message: 'Invalid category. Must be one of: ' + validCategories.join(', ') });
      }
    }
    ...
    const passage = await Passage.findById(passageId);
    if (!passage) {
      debugLog(`Passage not found for unassignment: ${passageId}`);
      return res.status(404).json({ message: 'Passage not found' });
    }
    ...
    if (passage.testTypes.length < originalLength) {
      ...
      debugLog(`Passage ${passageId} unassigned from categories: ${removedCategories.join(', ')}`);
    } else {
      debugLog(`Passage ${passageId} was not assigned to any of the requested categories`);
    }
