...
const { blogInteractionLimiter } = require('../middleware/rateLimiters');
...
// Track blog view
router.post('/:id/view', blogInteractionLimiter, async (req, res) => {
  try {
    const { timestamp, referrer } = req.body;
    ...
    // Use only server-controlled IP for uniqueness (not client-supplied User-Agent)
    const viewIdentifier = `${req.ip}-${blogId}`;
...
// Toggle blog like
router.post('/:id/like', blogInteractionLimiter, async (req, res) => {
  try {
    ...
    // Use only server-controlled IP for uniqueness (not client-supplied User-Agent)
    const likeIdentifier = `${req.ip}-${blogId}`;
