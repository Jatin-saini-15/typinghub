...
// NOTE: All AI API calls are proxied through the backend (/api/ai).
// The OpenRouter API key lives only in the backend .env (OPENROUTER_API_KEY).
// Do NOT set REACT_APP_OPENROUTER_API_KEY — it would expose the key in the public JS bundle.
export const AI_CONFIG = {
  // Base URL is for reference only; actual calls go through /api/ai (backend proxy)
